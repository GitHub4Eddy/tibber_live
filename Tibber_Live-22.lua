...
Changes New Tibber webSocket December 2022: 
...
local VERSION = "2.2"
-- FindMyiPhone/500 CFNetwork/758.4.3 Darwin/15.5.0
...
function QuickApp:updateButtonLabel(text) -- Update the label of the button 
  self:logging(3,"updateButtonLabel() - Update the label of the button")
...
function QuickApp:updateLabels() -- Update the labels
  ...
  if debugLevel == 4 then
    labelText = labelText .."OFFLINE SIMULATION MODE" .."\n\n"
  end
  if debugLevel == 5 then
    labelText = labelText .."LIVE TEST MODE" .."\n\n"
  end
...
function QuickApp:handleDataReceived(resp) -- Handle data received by the socket
  ...
  --elseif data.type == "data" then -- Tibber Live responded with data (old Tibber response)
...
function QuickApp:createVariables() -- Create all Variables 
  self:logging(3,"getData() - Create all Variables ")
  --url = "wss://api.tibber.com/v1-beta/gql/subscriptions" -- Old Tibber Live webSocket URL
  ...
  --headers["Sec-WebSocket-Protocol"] = "graphql-ws" -- Old Tibber header protocol
  headers["Sec-WebSocket-Protocol"] = "graphql-transport-ws" -- New Tibber header protocol
  headers["User-Agent"] = "Tibber_Live/2.2 Fibaro/HC3 Firmware/" ..api.get("/settings/info").softVersion -- New Tibber mandatory user-agent
